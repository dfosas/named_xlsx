...
* Implements an interface to work with named cells (workbook scope) in Excel:
  ...
  * with different backends in Python (`openpyxl`, `xlwings`, `calamine`).
...
To enable spreadsheet refresh through Excel via `xlwings`, install the optional extra:
...
Named table-column references are supported through workbook defined names
that point to a structured table reference. For example:
